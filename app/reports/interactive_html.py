"""Interactive HTML report + branded email HTML builders.

Ported from the original `core/html_builder.py` (incl. the GitHub "fixed tables"
commit), national-only:
  * build_interactive_html — standalone report: KPI cards, Dépôt KPI cards
    (CA split CRBT/Ordinaire), Dépôt & Livraison tables with click-to-drill
    MAILITM_FID panel, and Date-Dépôt + Date-Dernier-E range sliders.
  * build_email_html — branded email body (interactive HTML attached separately).
"""
...
from .branding import C_BG, C_LIGHT, C_NAVY, C_YELLOW


def get_logo_b64():
    return None  # no logo asset in the SQL-first app; header shows text only
...
def build_email_html(slicers, tables, timestamp, logo_b64=None, kpis=None, section_labels=None):
    if slicers:
        fi_rows = "".join(
        ...
        filters_html = ('<p style="color:#888;font-style:italic;margin:0;font-size:13px;">'
                        'Aucun filtre actif.</p>')
    ...
    if kpis:
        ...
        ca_s = (f"{total_ca:,.2f}".replace(",", " ") + " DT") if total_ca is not None else "—"
        kpi_email = (
            ...
            f'<div style="background:{C_YELLOW};border-radius:4px;height:7px;width:{bar_w}%;"></div></div></div></td>'
            ...
            f'<div style="color:#888;font-size:10px;">Intervalle dépôt → livraison</div></div></td>'
            f'<td width="25%" style="padding:0 3px;vertical-align:top;">'
            ...
            f'<div style="color:rgba(255,255,255,.6);font-size:10px;">identifiants uniques</div></div></td>'
            ...
            f'<div style="color:rgba(255,255,255,.6);font-size:10px;">chiffre d\'affaires</div></div></td>'
            f'</tr></table>')

    return f"""<!DOCTYPE html>
<html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:#EAEEF6;font-family:'Segoe UI',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#EAEEF6;padding:24px 0;"><tr><td align="center">
<table width="680" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(11,42,111,.15);max-width:680px;">
  <tr><td style="background:{C_NAVY};padding:0;">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:22px 30px 18px;vertical-align:middle;">
        <span style="color:#fff;font-size:20px;font-weight:700;letter-spacing:.5px;vertical-align:middle;">LA POSTE TUNISIENNE</span><br>
        <span style="color:{C_YELLOW};font-size:11px;letter-spacing:2px;text-transform:uppercase;">Rapport Automatisé &mdash; National</span>
      </td>
      <td style="padding:22px 30px 18px;text-align:right;white-space:nowrap;vertical-align:middle;">
        <span style="background:{C_YELLOW};color:{C_NAVY};padding:6px 14px;border-radius:20px;font-size:12px;font-weight:700;">{date_fr}</span>
      </td>
    </tr></table>
  ...
  <tr><td style="padding:32px 36px 24px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
      <tr><td style="background:{C_LIGHT};border-left:4px solid {C_NAVY};padding:14px 18px;border-radius:0 6px 6px 0;">
        <div style="color:{C_NAVY};font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;">Filtres appliqués</div>{filters_html}
      </td></tr>
    </table>
    <div style="color:{C_NAVY};font-size:14px;font-weight:700;border-bottom:2px solid {C_YELLOW};padding-bottom:8px;margin-bottom:22px;">Données détaillées</div>
    {tables_html}
    <p style="color:#888;font-size:12px;line-height:1.6;margin:24px 0 0;">
      Cordialement,<br><strong style="color:{C_NAVY};">Direction des Systèmes d'Information</strong><br>La Poste Tunisienne</p>
  </td></tr>
  <tr><td style="background:{C_NAVY};padding:0;">
    <div style="height:3px;background:{C_YELLOW};"></div>
    <table width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:18px 30px;text-align:center;">
      <div style="color:{C_YELLOW};font-size:13px;font-weight:700;letter-spacing:1px;margin-bottom:4px;">LA POSTE TUNISIENNE</div>
      <div style="color:rgba(255,255,255,.55);font-size:11px;">Direction des Systèmes d'Information &bull; Rapport généré automatiquement</div>
    </td></tr></table>
  </td></tr>
...
def _depot_kpis_html(depot_kpis):
    """Dépôt KPI cards per section (national-only renders just 'national')."""
    if not depot_kpis:
        return ""

    def _card_total(k):
        tid_s = f"{k.get('total', 0):,}".replace(",", " ")
        return (f'<div class="kpi-card" style="background:{C_NAVY};border-color:{C_NAVY};">'
                f'<div class="kpi-icon" style="font-size:24px;">&#128221;</div><div class="kpi-body">'
                f'<div class="kpi-label" style="color:{C_YELLOW};">Total colis</div>'
                f'<div class="kpi-val" style="color:#fff;">{tid_s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.6);">identifiants uniques</div></div></div>')

    def _card_ca(k):
        ca = k.get("ca", 0.0)
        ca_s = (f"{ca:,.0f}".replace(",", " ") + " DT") if ca else "—"
        return (f'<div class="kpi-card" style="background:#1A6B3A;border-color:#1A6B3A;">'
                f'<div class="kpi-icon" style="font-size:24px;">&#128176;</div><div class="kpi-body">'
                f'<div class="kpi-label" style="color:#A8E6C0;">CA Total</div>'
                f'<div class="kpi-val" style="color:#fff;">{ca_s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.6);">chiffre d\'affaires</div></div></div>')

    def _card_crbt_count(k):
        crbt_s = f"{k.get('crbt', 0):,}".replace(",", " ")
        return (f'<div class="kpi-card" style="background:#7B3F00;border-color:#7B3F00;">'
                f'<div class="kpi-icon" style="font-size:24px;">&#127981;</div><div class="kpi-body">'
                f'<div class="kpi-label" style="color:#FFD580;">CRBT</div>'
                f'<div class="kpi-val" style="color:#fff;">{crbt_s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.7);">colis contre remboursement</div></div></div>')

    def _card_ord_count(k):
        ord_s = f"{k.get('ordinaire', 0):,}".replace(",", " ")
        return (f'<div class="kpi-card"><div class="kpi-icon" style="font-size:24px;">&#128230;</div><div class="kpi-body">'
                f'<div class="kpi-label">Ordinaire</div><div class="kpi-val">{ord_s}</div>'
                f'<div class="kpi-sub">colis ordinaires</div></div></div>')

    def _card_ca_crbt(k):
        v = k.get("ca_crbt", 0.0)
        s = (f"{v:,.0f}".replace(",", " ") + " DT") if v else "—"
        return (f'<div class="kpi-card" style="background:#5A2D00;border-color:#5A2D00;">'
                f'<div class="kpi-icon" style="font-size:24px;">&#128176;</div><div class="kpi-body">'
                f'<div class="kpi-label" style="color:#FFD580;">CA CRBT</div>'
                f'<div class="kpi-val" style="color:#fff;">{s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.6);">chiffre d\'affaires CRBT</div></div></div>')

    def _card_ca_ordinaire(k):
        v = k.get("ca_ordinaire", 0.0)
        s = (f"{v:,.0f}".replace(",", " ") + " DT") if v else "—"
        return (f'<div class="kpi-card" style="background:#0D5C2E;border-color:#0D5C2E;">'
                f'<div class="kpi-icon" style="font-size:24px;">&#128176;</div><div class="kpi-body">'
                f'<div class="kpi-label" style="color:#A8E6C0;">CA Ordinaire</div>'
                f'<div class="kpi-val" style="color:#fff;">{s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.6);">chiffre d\'affaires ordinaire</div></div></div>')

    def _section(title, k):
        cards = (_card_total(k) + _card_ca(k) + _card_crbt_count(k)
                 + _card_ord_count(k) + _card_ca_crbt(k) + _card_ca_ordinaire(k))
        return (f'<div style="margin-bottom:18px;">'
                f'<div style="font-size:11px;font-weight:700;color:{C_NAVY};text-transform:uppercase;'
                f'letter-spacing:1.5px;margin-bottom:8px;border-left:3px solid {C_YELLOW};padding-left:8px;">{title}</div>'
                f'<div class="kpi-row">{cards}</div></div>')

    labels = {"national": "National", "export": "Export", "global": "Global"}
    body = "".join(_section(labels.get(key, key.title()), depot_kpis[key])
                   for key in ("national", "export", "global") if depot_kpis.get(key))
    return (f'<div style="margin:24px 0 20px;">'
            f'<div style="font-size:12px;font-weight:700;color:{C_NAVY};text-transform:uppercase;'
            f'letter-spacing:1px;margin-bottom:14px;border-left:4px solid {C_YELLOW};padding-left:10px;">KPIs Dépôt</div>'
            f'{body}</div>')


def _livraison_kpis_html(livraison_kpis):
    """Livraison KPI cards per section (national-only renders 'national' + 'global')."""
    ...
    def _card_taux(k):
        ...
        return (f'<div class="kpi-card"><div class="kpi-icon">&#128230;</div><div class="kpi-body">'
                f'<div class="kpi-label">Taux de livraison</div><div class="kpi-val">{taux_s}</div>'
                f'<div class="kpi-sub">{livres} livrés / {total} colis</div>'
                f'<div class="kpi-bar-bg"><div class="kpi-bar-fill" style="width:{bar_w}%"></div></div></div></div>')

    def _card_delai(k):
        avg = k.get("avg_intervalle")
        avg_s = f"{avg} j" if avg is not None else "—"
        return (f'<div class="kpi-card"><div class="kpi-icon">&#9201;</div><div class="kpi-body">'
                f'<div class="kpi-label">Délai moyen</div><div class="kpi-val">{avg_s}</div>'
                f'<div class="kpi-sub">Intervalle moyen en jours</div></div></div>')
    ...
    def _card_crbt(k):
        ...
        return (f'<div class="kpi-card" style="background:#7B3F00;border-color:#7B3F00;">'
                ...
                f'<div class="kpi-label" style="color:#FFD580;">CRBT</div>'
                f'<div class="kpi-val" style="color:#fff;">{crbt_s}</div>'
                f'<div class="kpi-sub" style="color:rgba(255,255,255,.7);">Ordinaire : {ord_s}</div></div></div>')

    def _section(title, k, show_delai=True):
        cards = (_card_taux(k) + (_card_delai(k) if show_delai else "")
                 + _card_total(k) + _card_ca(k) + _card_crbt(k))
        return (f'<div style="margin-bottom:18px;">'
                f'<div style="font-size:11px;font-weight:700;color:{C_NAVY};text-transform:uppercase;'
                f'letter-spacing:1.5px;margin-bottom:8px;border-left:3px solid {C_YELLOW};padding-left:8px;">{title}</div>'
                f'<div class="kpi-row">{cards}</div></div>')

    labels = {"national": "National", "import": "Import", "global": "Global"}
    body = "".join(_section(labels.get(key, key.title()), livraison_kpis[key], show_delai=(key != "global"))
                   for key in ("national", "import", "global") if livraison_kpis.get(key))
    return (f'<div style="margin:24px 0 20px;">'
            f'<div style="font-size:12px;font-weight:700;color:{C_NAVY};text-transform:uppercase;'
            f'letter-spacing:1px;margin-bottom:14px;border-left:4px solid {C_YELLOW};padding-left:10px;">KPIs Livraison</div>'
            f'{body}</div>')


def build_interactive_html(slicers, tables, drill_mappings, timestamp, logo_b64=None,
                           kpis=None, auto_refresh=False, section_labels=None,
                           depot_kpis=None, livraison_kpis=None,
                           liv_slider_data=None, dep_slider_data=None):
    fi = ""
    if slicers:
        _title_map = {"Region Depot": "Région", "Region Next": "Région de destination",
                      "Region dernier E": "Région livraison"}
        items = "".join(
            f'<li><strong style="color:{C_NAVY};">{_title_map.get(s["title"], s["title"]) or "Filtre"}</strong>'
    ...
    kpi_html = ""
    if kpis and not livraison_kpis:
        taux = kpis.get("taux", 0)
        livres = kpis.get("livres", 0)
        total = kpis.get("total", 0)
        total_ids = kpis.get("total_ids", total)
        ...
        kpi_html = (
            f'<div class="kpi-row">'
            f'<div class="kpi-card"><div class="kpi-icon">&#128230;</div><div class="kpi-body">'
            f'<div class="kpi-label">Taux de livraison</div><div class="kpi-val">{taux}%</div>'
            f'<div class="kpi-sub">{livres} livrés / {total} envois</div>'
            f'<div class="kpi-bar-bg"><div class="kpi-bar-fill" style="width:{bar_w}%"></div></div></div></div>'
            f'<div class="kpi-card"><div class="kpi-icon">&#9201;</div><div class="kpi-body">'
            f'<div class="kpi-label">Délai moyen</div><div class="kpi-val">{avg_s}</div>'
            f'<div class="kpi-sub">Intervalle dépôt → livraison</div></div></div>'
    ...
    _first_liv_idx = next((n for n in range(len(tables))
                           if section_labels and section_labels.get(n) == "LIVRAISON"), None)
    ...
    _first_dep_idx = next((n for n in range(len(tables))
                           if section_labels and section_labels.get(n) == "DEPOT"), None)
    _show_dep_slider = bool(dep_slider_data and any(r.get("d") for r in dep_slider_data)
                            and _first_dep_idx is not None)
    _first_dep_tk = f"t{_first_dep_idx}" if _first_dep_idx is not None else ""

    th_html = ""
    for n, t in enumerate(tables):
        if section_labels and n in section_labels:
            lbl = section_labels[n]
            th_html += f'<div class="section-hdr"><span class="section-hdr-text">{lbl}</span></div>'
            if lbl == "DEPOT" and depot_kpis_html:
                th_html += depot_kpis_html
            if lbl == "DEPOT" and _show_dep_slider:
                th_html += (
                    f'<div class="liv-slider-box">'
                    f'<div class="liv-slider-title">&#128197; Filtrer par Date Dépôt</div>'
                    f'<div class="liv-slider-row">'
                    f'<label>Du&nbsp;<input type="date" id="dep-d-min" onchange="applyDepFilter()"></label>'
                    f'<span class="liv-sep">&#8594;</span>'
                    f'<label>Au&nbsp;<input type="date" id="dep-d-max" onchange="applyDepFilter()"></label>'
                    f'<button class="liv-reset-btn" onclick="resetDepFilter()">R&#233;initialiser</button>'
                    f'<span id="dep-filter-info" class="liv-filter-info"></span></div></div>')
            ...
            if lbl == "LIVRAISON" and _show_slider:
                th_html += (
                    ...
                    f'<span id="liv-filter-info" class="liv-filter-info"></span></div></div>')
        tk = f"t{n}"
        ...
        for ci in range(len(t["headers"])):
            ...
            for ri, row in enumerate(t["rows"]):
                if ri in pbi_tot_idx:
                    continue
                if ci < len(row):
                    try:
                        vals.append(float(row[ci].replace(" ", "").replace(" ", "").replace(",", ".")))
                    except Exception:
                        pass
            if vals:
                col_totals[ci] = str(int(round(sum(vals))))
        th = "".join(f'<th>{h}</th>' for h in t["headers"])

        # Sub-keys present in drill data (union across dims, excluding __all__)
        _valid_sub_keys = set()
        if dm_data:
            for _dd in dm_data.values():
                if isinstance(_dd, dict):
                    _valid_sub_keys.update(_dd.keys())
            _valid_sub_keys.discard("__all__")
        ...
        for ri, row in enumerate(t["rows"]):
            ...
            for ci, val in enumerate(row):
                ...
                if is_tot:
                    ...
                    elif "total" not in hdr.lower() and hdr in _valid_sub_keys:
        ...
        tfoot = ""  # TOTAL row is already part of the table body
        _is_first_liv = (_show_slider and n == _first_liv_idx)
        _is_first_dep = (_show_dep_slider and n == _first_dep_idx)
        if _is_first_dep:
            _head_id, _body_id, _meta_id = ' id="dep-nat-head"', ' id="dep-nat-body"', ' id="dep-nat-meta"'
        elif _is_first_liv:
            _head_id, _body_id, _meta_id = ' id="liv-nat-head"', ' id="liv-nat-body"', ' id="liv-nat-meta"'
        else:
            _head_id = _body_id = _meta_id = ""
        th_html += (
            f'<h3>{label}{hint}</h3>'
    ...
    dep_raw_json = json.dumps(dep_slider_data or [], ensure_ascii=False) if _show_dep_slider else "[]"
    date_fr = datetime.strptime(timestamp, "%Y-%m-%d %H:%M").strftime("%d/%m/%Y à %H:%M")
    logo_tag = (f'<img src="{logo_b64}" alt="La Poste Tunisienne" '
                f'style="height:48px;margin-right:14px;vertical-align:middle;">' if logo_b64 else "")
...
<div class="wrapper">
  <div class="page-header"><div class="top">
...
<div id="panel">
  ...
  <div style="overflow-x:auto"><table id="ptable"><thead id="phead"></thead><tbody id="pbody"></tbody></table></div>
</div>
<script>
const D=DRILL_DATA;
function drill(tk,dv,colH){{
  ...
  if(rows.length){{
    document.getElementById('pbody').innerHTML=rows.map(r=>{{
      ...
      const extras=cols.map(c=>'<td>'+(r[c]!==undefined?r[c]:'')+'</td>').join('');
...
var LIV_TK='{_liv_tk_js}';
const _livDs=LIV_RAW.map(r=>r.d).filter(Boolean).sort();
const LIV_MIN=_livDs[0]||'';
const LIV_MAX=_livDs[_livDs.length-1]||'';
document.addEventListener('click',function(e){{
  var td=e.target.closest?e.target.closest('td[data-lbur]'):null;
  if(!td)return;
  var bur=td.getAttribute('data-lbur');
  var col=td.getAttribute('data-lcol')||undefined;
  if(bur&&typeof drill!=='undefined')drill(LIV_TK,bur,col);
}});
window.addEventListener('load',function(){{
  var mn=document.getElementById('liv-d-min');var mx=document.getElementById('liv-d-max');
  if(!mn)return; mn.value=LIV_MIN; mx.value=LIV_MAX; applyLivFilter();
}});
function applyLivFilter(){{
  var mn=(document.getElementById('liv-d-min')||{{}}).value||'';
  var mx=(document.getElementById('liv-d-max')||{{}}).value||'';
  var fil=LIV_RAW.filter(function(r){{if(!r.d)return true;return(!mn||r.d>=mn)&&(!mx||r.d<=mx);}});
  rebuildLivTable(fil,mn,mx);
}}
function resetLivFilter(){{
  var mn=document.getElementById('liv-d-min');var mx=document.getElementById('liv-d-max');
  if(mn)mn.value=LIV_MIN; if(mx)mx.value=LIV_MAX; applyLivFilter();
}}
function rebuildLivTable(data,mn,mx){{
  var bHead=document.getElementById('liv-nat-head');var bBody=document.getElementById('liv-nat-body');
  ...
  var hdrs=['Bureau Dernier E',...deVals,'Total IDs','CRBT','Ordinaire','CA (DT)','Poids'];
  bHead.innerHTML='<tr>'+hdrs.map(h=>'<th>'+h+'</th>').join('')+'</tr>';
  var tIds=0,tCrbt=0,tOrd=0,tCa=0.0,tPds=0.0;var tDe={{}};deVals.forEach(v=>{{tDe[v]=0;}});
  var C_NAV='{C_NAVY}',C_BG2='{C_BG}';
  var bodyHtml=bureaux.map(function(bur,i){{
    var sub=data.filter(r=>r.b===bur);var nb=sub.length;
    var cr=sub.filter(r=>r.t==='CRBT').length;var or_=nb-cr;
    var ca=sub.reduce((s,r)=>s+(r.c||0),0);
    var pds=sub.reduce((s,r)=>s+(r.p||0),0);
    var dCts={{}};deVals.forEach(v=>{{dCts[v]=sub.filter(r=>r.e===v).length;tDe[v]+=dCts[v];}});
    tIds+=nb;tCrbt+=cr;tOrd+=or_;tCa+=ca;tPds+=pds;
    var bg=i%2===0?'#fff':C_BG2;
    var caS=Math.round(ca).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
    var pdsS=Math.round(pds).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
    var bur_attr=bur.replace(/&/g,'&amp;').replace(/"/g,'&quot;');
    var cells='<td class="cl dim-cell" style="cursor:pointer;color:'+C_NAV+';font-weight:600;" data-lbur="'+bur_attr+'">'+bur+'</td>';
    deVals.forEach(v=>{{var v_attr=v.replace(/&/g,'&amp;').replace(/"/g,'&quot;');
      cells+='<td class="cl dat-cell" style="background:'+bg+';cursor:pointer;" data-lbur="'+bur_attr+'" data-lcol="'+v_attr+'">'+(dCts[v]?dCts[v]:'&mdash;')+'</td>';}});
    cells+='<td class="cl" style="background:'+bg+';cursor:pointer;" data-lbur="'+bur_attr+'">'+nb+'</td>';
    cells+='<td class="cl dat-cell" style="background:'+bg+';cursor:pointer;" data-lbur="'+bur_attr+'" data-lcol="CRBT">'+cr+'</td>';
    cells+='<td class="cl dat-cell" style="background:'+bg+';cursor:pointer;" data-lbur="'+bur_attr+'" data-lcol="Ordinaire">'+or_+'</td>';
    cells+='<td style="background:'+bg+';color:#1A6B3A;font-weight:600;">'+caS+'</td>';
    cells+='<td style="background:'+bg+';">'+pdsS+'</td>';
    return '<tr>'+cells+'</tr>';
  }}).join('');
  var tCaS=Math.round(tCa).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
  var tPdsS=Math.round(tPds).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
  ...
  totCells+='<td class="tot-cell">'+tOrd+'</td><td class="tot-cell">'+tCaS+'</td><td class="tot-cell">'+tPdsS+'</td>';
  bBody.innerHTML=bodyHtml+'<tr>'+totCells+'</tr>';
  if(bMeta){{var rng=(mn&&mx)?' | '+mn.split('-').reverse().join('/')+' → '+mx.split('-').reverse().join('/'):'';bMeta.textContent=data.length+' colis'+rng;}}
  var fi=document.getElementById('liv-filter-info');if(fi)fi.textContent=data.length+' / '+LIV_RAW.length+' colis';
}}"""
    else:
        _slider_js = ""

    if _show_dep_slider:
        _dep_tk_js = _first_dep_tk
        _dep_slider_js = f"""
const DEP_RAW={dep_raw_json};
var DEP_TK='{_dep_tk_js}';
const _depDs=DEP_RAW.map(r=>r.d).filter(Boolean).sort();
const DEP_MIN=_depDs[0]||'';
const DEP_MAX=_depDs[_depDs.length-1]||'';
document.addEventListener('click',function(e){{
  var td=e.target.closest?e.target.closest('td[data-dbur]'):null;
  if(!td)return;
  var bur=td.getAttribute('data-dbur');
  var col=td.getAttribute('data-dcol')||undefined;
  if(bur&&typeof drill!=='undefined')drill(DEP_TK,bur,col);
}});
window.addEventListener('load',function(){{
  var mn=document.getElementById('dep-d-min');var mx=document.getElementById('dep-d-max');
  if(!mn)return; mn.value=DEP_MIN; mx.value=DEP_MAX; applyDepFilter();
}});
function applyDepFilter(){{
  var mn=(document.getElementById('dep-d-min')||{{}}).value||'';
  var mx=(document.getElementById('dep-d-max')||{{}}).value||'';
  var fil=DEP_RAW.filter(function(r){{if(!r.d)return true;return(!mn||r.d>=mn)&&(!mx||r.d<=mx);}});
  rebuildDepTable(fil,mn,mx);
}}
function resetDepFilter(){{
  var mn=document.getElementById('dep-d-min');var mx=document.getElementById('dep-d-max');
  if(mn)mn.value=DEP_MIN; if(mx)mx.value=DEP_MAX; applyDepFilter();
}}
function rebuildDepTable(data,mn,mx){{
  var bHead=document.getElementById('dep-nat-head');var bBody=document.getElementById('dep-nat-body');
  var bMeta=document.getElementById('dep-nat-meta');
  if(!bHead||!bBody)return;
  var bureaux=[...new Set(data.map(r=>r.b))].filter(Boolean).sort();
  var hdrs=['Bureau dépôt','CRBT','CA','Poids','Nb IDs'];
  bHead.innerHTML='<tr>'+hdrs.map(h=>'<th>'+h+'</th>').join('')+'</tr>';
  var tIds=0,tCrbt=0,tCa=0.0,tPds=0.0;
  var C_NAV='{C_NAVY}',C_BG2='{C_BG}';
  var bodyHtml=bureaux.map(function(bur,i){{
    var sub=data.filter(r=>r.b===bur);var nb=sub.length;
    ...
    var ca=sub.reduce((s,r)=>s+(r.c||0),0);
    var pds=sub.reduce((s,r)=>s+(r.p||0),0);
    tIds+=nb;tCrbt+=cr;tCa+=ca;tPds+=pds;
    var bg=i%2===0?'#fff':C_BG2;
    var caS=Math.round(ca).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
    var pdsS=Math.round(pds).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
    var bur_attr=bur.replace(/&/g,'&amp;').replace(/"/g,'&quot;');
    var cells='<td class="cl dim-cell" style="cursor:pointer;color:'+C_NAV+';font-weight:600;" data-dbur="'+bur_attr+'">'+bur+'</td>';
    cells+='<td class="cl dat-cell" style="background:'+bg+';cursor:pointer;" data-dbur="'+bur_attr+'" data-dcol="CRBT">'+cr+'</td>';
    cells+='<td style="background:'+bg+';color:#1A6B3A;font-weight:600;">'+caS+'</td>';
    cells+='<td style="background:'+bg+';">'+pdsS+'</td>';
    cells+='<td class="cl" style="background:'+bg+';cursor:pointer;" data-dbur="'+bur_attr+'">'+nb+'</td>';
    return '<tr>'+cells+'</tr>';
  }}).join('');
  var tCaS=Math.round(tCa).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
  var tPdsS=Math.round(tPds).toString().replace(/\\B(?=(\\d{{3}})+(?!\\d))/g,' ');
  var totCells='<td class="tot-cell">TOTAL</td><td class="tot-cell">'+tCrbt+'</td>'
    +'<td class="tot-cell">'+tCaS+'</td><td class="tot-cell">'+tPdsS+'</td><td class="tot-cell">'+tIds+'</td>';
  bBody.innerHTML=bodyHtml+'<tr>'+totCells+'</tr>';
  if(bMeta){{var rng=(mn&&mx)?' | '+mn.split('-').reverse().join('/')+' → '+mx.split('-').reverse().join('/'):'';bMeta.textContent=data.length+' colis'+rng;}}
  var fi=document.getElementById('dep-filter-info');if(fi)fi.textContent=data.length+' / '+DEP_RAW.length+' colis';
}}"""
    else:
        _dep_slider_js = ""
    ...
    return (html.replace("KPIS_BLOCK", kpi_html)
                ...
                .replace("DEP_SLIDER_JS", _dep_slider_js))
